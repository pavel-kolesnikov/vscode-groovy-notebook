...
function deserializeCellOutputs(
    outputs: SerializedCellOutput[],
    ...
    return outputs.map((output) => {
        const outputItems =
            output.outputs?.map((item) => {
                ...
                const encodedValue: Uint8Array = new TextEncoder().encode(
                    String(item.value),
                );
                return new vscode.NotebookCellOutputItem(
                    encodedValue,
...
function serializeCellOutputs(
    outputs: vscode.NotebookCellOutput[],
    compress: boolean,
): SerializedCellOutput[] {
    return outputs.map((output) => ({
...
export class GroovyContentSerializer implements vscode.NotebookSerializer {
    ...
    public async deserializeNotebook(
        ...
        try {
            ...
            console.error(
                '[GroovyContentSerializer] Failed to parse notebook:',
                e,
            );
            raw = { cells: [] };
        }
        ...
        if (schemaVersion && schemaVersion !== SCHEMA_VERSION) {
            console.warn(
                `[GroovyContentSerializer] Schema version mismatch: expected ${SCHEMA_VERSION}, got ${schemaVersion}`,
            );
        }
    ...
    public async serializeNotebook(
        ...
        for (const cell of data.cells) {
            const cellId =
                (cell.metadata as Record<string, string>)?._cellId ||
                generateCellId();
