...
export { ConfigValidationResult, ValidationResult };
...
export function getGroovyPath(): string {
    return (
        (vscode.workspace
            .getConfiguration()
            .get(CONFIG.GROOVY_PATH_KEY) as string) ||
        CONFIG.DEFAULT_GROOVY_PATH
    );
}
...
export function getJavaHome(): string | undefined {
    const javaHome = vscode.workspace
        .getConfiguration()
        .get(CONFIG.JAVA_HOME_KEY) as string;
...
export function shouldCompressOutputs(): boolean {
    return (
        vscode.workspace
            .getConfiguration()
            .get<boolean>(CONFIG.COMPRESS_OUTPUTS_KEY) ?? true
    );
}
