...
import { GroovyProcess, type ProcessResult } from './process.js';
import type { Executable, ExecutionStatus, ProcessConfig } from './types.js';
...
export class GroovySession implements vscode.Disposable, Executable {
    ...
    public async ensureStarted(): Promise<void> {
        ...
        if (!this.process) {
            log(
                'Session',
                `ensureStarted: creating new process for ${this.notebookUri.toString()}`,
            );
    ...
    public async run(
        ...
        log(
            'Session',
            `run: called with code length=${code.length}, current process=${this.process ? 'exists' : 'null'}`,
        );
        ...
        if (!process) {
            throw new Error('Session process failed to start');
        }
        ...
        try {
            const result = await process.run(code, onOutput);
            log(
                'Session',
                `run: code executed successfully, stdout length=${result.stdout?.length || 0}`,
            );
            this.setStatus('idle');
            return result;
        } catch (error) {
    ...
    private async createProcess(): Promise<GroovyProcess> {
        ...
        this.processStatusSubscription = proc.onDidChangeStatus(
...
export class SessionRegistry implements vscode.Disposable {
    ...
    private readonly sessionSubscriptions = new Map<
        ...
        vscode.Disposable
    >();
    private readonly onDidChangeSessionStatus = new vscode.EventEmitter<{
        uri: vscode.Uri;
        status: SessionStatus;
    }>();
    private readonly onDidRestartSession =
    ...
    public async disposeSession(uri: vscode.Uri): Promise<void> {
        const key = uri.toString();
        const session = this.sessions.get(key);
        if (!session) return;
        this.sessionSubscriptions.get(key)?.dispose();
