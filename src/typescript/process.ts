...
import { SIGNAL_END_OF_MESSAGE, SIGNAL_READY } from './protocol.js';
import type {
    ExecutionError,
    ExecutionResult,
    ExecutionStatus,
    ...
    private spawn(): Promise<ChildProcess> {
        return new Promise((resolve, reject) => {
            ...
            if (!stdout) {
                ...
                reject(
                    new Error(
                        `Groovy process closed unexpectedly during startup. This may indicate a problem with the Groovy installation or the Groovy Kernel script.`,
                    ),
                );
                return;
            }
            ...
            const timeoutId = setTimeout(() => {
                ...
                log(
                    'Process',
                    `TIMEOUT: No SIGNAL_READY received after ${GroovyProcess.INITIALIZATION_TIMEOUT / 1000}s`,
                );
                this.killProcess(proc);
                reject(
                    new Error(
                        `Failed to start Groovy shell after ${GroovyProcess.INITIALIZATION_TIMEOUT / 1000}s. Verify Groovy is installed and accessible at '${this.config.groovyPath}'. Run 'groovy --version' in terminal to check.`,
                    ),
                );
            ...
            proc.on('exit', (code, signal) => {
                log(
                    'Process',
                    `Process exit event: code=${code}, signal=${signal}, readyReceived=${readyReceived}`,
                );
            });
    ...
    private executeCode(
        ...
        return new Promise((resolve, reject) => {
            ...
            const onExit = (code: number | null) => {
                ...
                log(
                    'Process',
                    'executeCode: process exited during execution, code=',
                    code,
                );
            ...
            const onStdout = (chunk: Buffer) => {
                log(
                    'Process',
                    'executeCode: stdout chunk:',
                    formatBuffer(chunk, CONFIG.LOG_PREVIEW_LONG_LENGTH),
                );
                ...
                if (chunk.includes(SIGNAL_END_OF_MESSAGE)) {
                    ...
                    log(
                        'Process',
                        'executeCode: SIGNAL_END_OF_MESSAGE received, resolving',
                    );
            ...
            log(
                'Process',
                'executeCode: writing to stdin, bytes:',
                message.length,
            );
